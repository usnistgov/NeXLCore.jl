...
dEds(ty::Type{<:BetheEnergyLoss}, e::Float64, mat::Material, mip::Type{<:NeXLMeanIonizationPotential}=Berger1982) =
    mapreduce(elm->dEds(ty, e, elm, density(mat))*mat[elm], +, keys(mat))


Base.range(ty::Type{<:BetheEnergyLoss}, mat::Material, e0::Float64, emin=50.0, mip::Type{<:NeXLMeanIonizationPotential}=Berger1982) =
    quadgk(e->1.0/dEds(ty, e, mat, mip), e0, emin, rtol=1.0e-6)[1]

struct Kanaya1972 end

function Base.range(::Type{Kanaya1972}, mat::Material, e0::Float64)
    ko(elm, e0) = 0.0276 * a(elm) * (0.001*e0)^1.67 / z(elm)^0.89
    return (1.0e-4 / mapreduce(elm->mat[elm]/ko(elm,e0), +, keys(mat))) / density(mat)
end
