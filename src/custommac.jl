...
let custommacs = nothing
    global function getcustommacs()
        if isnothing(custommacs)
            ...
            for row in CSV.File(joinpath(dirname(pathof(@__MODULE__)),"..","data","specialmacs.csv"), header=1)
...
function getcustommacs(model::Symbol, withsimilar=true)::Vector{Tuple{Element, CharXRay, Float64}}
    macs = getcustommacs()
    tmp = [ ( elements[key[2]], key[3], macs[key] ) for key in filter(k->k[1]==model, keys(macs)) ]
    if withsimilar
        res = []
        for macs in tmp
...
mac(elm::Element, cxr::CharXRay, ::Type{CustomMAC}; alg=:Bastin1989)::Union{Missing, Float64} =
    getcustommac(elm, cxr, alg)
