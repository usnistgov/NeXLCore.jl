...
const LiljequistCorrection = begin
    ...
    es = 1000.0 * [ 0.1,0.15,0.2,0.3,0.4,0.5,0.7,1,1.5,2,3,4,5,7,10,15,20,30,40,50,70,100 ] # eV
    tbl3 = [ 1.257 1.211 1.188 1.165 1.153 1.147 1.139 1.134 1.129 1.127 1.123 1.121 1.119 1.116 1.113 1.111 1.11 1.11 1.11 1.112 1.115 1.119;
    ...
    [ Spline1D(es, [ zspl[ie](Float64(z)) for ie in eachindex(es)], k=3) for z in 1:92 ]
end
...
function σₜ(::Type{Browning1991}, elm::Element, E::Float64)
    ...
    return 4.7e-18*(z(elm)^1.33+0.032*z(elm)^2)/((e+0.0155*(z(elm)^1.33)*sqrt(e))*(1.0-0.02*sqrt(z(elm))*exp(-u^2)))
end
function σₜ(::Type{Browning1994}, elm::Element, E::Float64)
    e = 0.001*E
    return 3.0e-18*z(elm)^1.7/(e+0.005*z(elm)^1.7*sqrt(e)+0.0007*z(elm)^2/sqrt(e))
end
...
"""
function λₜᵣ(ty::Type{<:ElasticScatteringCrossSection}, elm::Element, E::Float64, N::Float64)
